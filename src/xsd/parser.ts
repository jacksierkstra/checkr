...
export class XSDParserImpl implements XSDParser {
  ...
  /**
   * If an <xs:simpleType> has <xs:restriction> with <xs:enumeration> children,
   * collect those enumerated values into a string[].
   */
  ...
  /**
   * Parses <xs:attribute> nodes from an <xs:element>.
   */
  ...
  // 1) Modify parseNestedElements to return { children, choices }
  ...
  /**
   * Creates an XSDChoice object by collecting all <xs:element> children of <xs:choice>.
   */
