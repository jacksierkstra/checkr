 ...
 */
export const validateRootElements = (xmlDoc: XMLDocument, schema: XSDSchema): ValidationError[] => {
  ...
  for (const element of schema.elements) {
    ...
    if (matchingNodes.length < minOccurs) {
      errors.push({
        ...
        message: `Root element <${element.name}> is required in the document but ${
