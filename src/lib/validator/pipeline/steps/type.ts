...
export const validateType: NodeValidationStep = (node, schema) => {
  ...
  if (schema.pattern) {
    try {
      ...
      errors.push(
        `Element <${schema.name}> has an invalid pattern "${schema.pattern}" in the schema definition.`,
      );
    }
  }
  ...
  switch (schema.type) {
    ...
    case "xs:boolean":
      if (!["true", "false", "1", "0"].includes(text)) {
        errors.push(
          `Element <${schema.name}> must be a boolean (true/false/1/0), but found "${text}".`,
        );
      }
      break;
    case "xs:date":
      if (!/^\d{4}-\d{2}-\d{2}$/.test(text)) {
        errors.push(
          `Element <${schema.name}> must be a valid date (YYYY-MM-DD), but found "${text}".`,
        );
      }
      break;
    default:
 ...
 */
function validateNumericConstraints(value: number, schema: XSDElement, errors: string[]): void {
