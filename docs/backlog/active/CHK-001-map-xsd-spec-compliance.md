...
- [XSTS-derived regression corpus](docs/backlog/archive/CHK-028-xsts-derived-regression-corpus.md) — 64 derived schema/instance pairs across 34 feature areas covering all structures and datatypes from the gap analysis volume table, keyed to XSTS manifest files. Runs as part of the test suite; reports pass/fail counts per area via a summary table. Three bugs exposed and fixed: `\c` regex escape was missing letters (explains why `http://\c*` couldn't match `www.foo.com`), `minOccurs` on single-element particles unenforced for partial matches, and `<xs:group ref>` as complexType compositor silently ignored. All gap analysis "unknown" levels converted to measured levels (`full` for all 34 areas).
